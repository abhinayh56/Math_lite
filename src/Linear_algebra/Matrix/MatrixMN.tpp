...
template <typename T, size_t M, size_t N>
inline constexpr MatrixMN<T, M, N> MatrixMN<T, M, N>::trans(const MatrixMN<T, M, N> &M1)
{
...
template <typename T, size_t M, size_t N>
inline constexpr T MatrixMN<T, M, N>::det(const MatrixMN<T, M, N> &M1)
{
            ...
            if (M1.m[0][j] != T(0))
            {
                ...
                for (size_t row = 1; row < M; ++row)
                    for (size_t col = 0, subcol = 0; col < N; ++col)
                        if (col != j)
                            submatrix.m[row - 1][subcol++] = M1.m[row][col];
                result += sign * M1.m[0][j] * det(submatrix);
            }
            sign = -sign;
        }
        return result;
...
template <typename T, size_t M, size_t N>
inline constexpr T MatrixMN<T, M, N>::trace(const MatrixMN<T, M, N> &M1)
{
...
template <typename T, size_t M, size_t N>
inline constexpr MatrixMN<T, M, N> MatrixMN<T, M, N>::minor(const MatrixMN<T, M, N> &M1)
{
        ...
        for (size_t j = 0; j < N; ++j)
        {
            ...
            for (size_t row = 0, subrow = 0; row < M; ++row)
            {
                if (row != i)
                {
                    for (size_t col = 0, subcol = 0; col < N; ++col)
                        if (col != j)
                            submatrix.m[subrow][subcol++] = M1.m[row][col];
                    ++subrow;
                }
            }
...
template <typename T, size_t M, size_t N>
inline constexpr MatrixMN<T, M, N> MatrixMN<T, M, N>::cofactor(const MatrixMN<T, M, N> &M1)
{
    static_assert(M == N, "Cofactor is only defined for square matrices");
    MatrixMN<T, M, N> result = minor(M1);
    for (size_t i = 0; i < M; ++i)
        for (size_t j = 0; j < N; ++j)
            if ((i + j) % 2 == 1)
...
template <typename T, size_t M, size_t N>
inline constexpr MatrixMN<T, M, N> MatrixMN<T, M, N>::adj(const MatrixMN<T, M, N> &M1)
{
    static_assert(M == N, "Adjugate is only defined for square matrices");
    return trans(cofactor(M1));
}

template <typename T, size_t M, size_t N>
inline constexpr MatrixMN<T, M, N> MatrixMN<T, M, N>::inv(const MatrixMN<T, M, N> &M1)
{
